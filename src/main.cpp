...
// I2S bus driving the Pmod I2S2 DAC.
I2SClass i2s;
...
void setup() {
    ...
    // Configure I2S pins: bclk, ws, dout, din (unused → -1), mclk.
    i2s.setPins(PIN_SCLK, PIN_LRCK, PIN_SDIN, -1, PIN_MCLK);

    if (i2s.begin(I2S_MODE_STD, SYNTH_AUDIO_RATE, I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO)) {
        ...
        if (reading == stableState[i]) {
            ...
            if (reading == LOW) {
                Serial.printf("Button %d (GPIO %d) pressed  → %.2f Hz\n",
                              i + 1, BUTTON_PINS[i], NOTE_FREQS[i]);
                bank.triggerOn(i);
            } else {
